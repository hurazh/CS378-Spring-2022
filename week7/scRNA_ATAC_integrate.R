...
# nBiocManager::install("biovizBase")
...
pbmc.rna <- LoadData("pbmcMultiome", "pbmc.rna") # scRNA
pbmc.atac <- LoadData("pbmcMultiome", "pbmc.atac") # ATAC


# QC
pbmc.rna <- subset(pbmc.rna, seurat_annotations != "filtered")
pbmc.atac@assays$ATAC@key <- "atac_" # workaround of some weird Seurat dependency problem
...
p3 <- DimPlot(pbmc.atac, group.by = "predicted.id", label = TRUE) + NoLegend() + ggtitle("Predicted annotation")
p4 <- DimPlot(pbmc.atac, group.by = "seurat_annotations", label = TRUE) + NoLegend() + ggtitle("Ground-truth annotation")
p3 | p4

predictions <- table(pbmc.atac$seurat_annotations, pbmc.atac$predicted.id)
predictions <- predictions/rowSums(predictions)  # normalize for number of cells in each cell type
predictions <- as.data.frame(predictions)
p5 <- ggplot(predictions, aes(Var1, Var2, fill = Freq)) + geom_tile() + scale_fill_gradient(name = "Fraction of cells",
...
p6 <- ggplot(data, aes(prediction.score.max, fill = annotation_correct, colour = annotation_correct)) +
